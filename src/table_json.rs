...
use sporm::table::{Table,Column,Foreign};
use sporm::dao::{Value,Type};
use serde_json;
use serde_json::Value as Json;
use std::error::Error;
use std::fmt;
use sporm::platform::postgres::Postgres;
...
impl TableJson for Table{
            ...
            &Json::Object(ref json_column) => {
                ...
                let default = match json_column.get("default"){
                    Some(default) => match default{
                        ...
                        &Json::Bool(default) => Some(Operand::Value(Value::Bool(default))),
    ...
    fn to_json(&self)->String{
        ...
        format!("{}", serde_json::to_string_pretty(&json_object).unwrap())
    }
    ...
    fn column_to_btree(column: &Column)->Json{
        ...
        if column.is_primary{
            btree.insert("primary".to_owned(), Json::Bool(column.is_primary));
        }
        if column.is_unique{
            btree.insert("unique".to_owned(), Json::Bool(column.is_unique));
        }
        // don't include default values
        if !column.nullable(){
            btree.insert("nullable".to_owned(), Json::Bool(column.nullable()));
        }
